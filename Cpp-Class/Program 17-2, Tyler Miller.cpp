...
Modified by Tyler Miller, 5 AUG 2019.
-> Functions added to perform arithmetic on fractions and to do decimal conversions.
Using examples and principles from "Jumping into C++" by Alex Allain, and materials covered in this course.
*/
#include <iostream>
#include <string>
#include <iomanip>
...
void printDecimals(FractionArray fractions1, FractionArray fractions2);

double fracToDec1(Fraction op1);
double fracToDec2(Fraction op2);
int gcd(int x, int y);
...
int main(int argc, char *argv[])
{
	...
	printDecimals(fractions1, fractions2);
	...
	{
		Fraction result = add(fractions1.fractions[i], fractions2.fractions[i]);
		cout << tostring(fractions1.fractions[i]) << " + " << tostring(fractions2.fractions[i])
			<< " = " << tostring(result) << endl;
	}
	cout << '\n';
	for (int i = 0; i < fractions1.nFractions; i++)
	{
		Fraction result = subtract(fractions1.fractions[i], fractions2.fractions[i]);
		cout << tostring(fractions1.fractions[i]) << " - " << tostring(fractions2.fractions[i])
			<< " = " << tostring(result) << endl;
	}
	cout << '\n';
	for (int i = 0; i < fractions1.nFractions; i++)
	{
		Fraction result = multiply(fractions1.fractions[i], fractions2.fractions[i]);
		cout << tostring(fractions1.fractions[i]) << " * " << tostring(fractions2.fractions[i])
			<< " = " << tostring(result) << endl;
	}
	cout << '\n';
	for (int i = 0; i < fractions1.nFractions; i++)
	{
		Fraction result = divide(fractions1.fractions[i], fractions2.fractions[i]);
		cout << tostring(fractions1.fractions[i]) << " ÷ " << tostring(fractions2.fractions[i])
			<< " = " << tostring(result) << endl;
	}
...
Fraction multiply(Fraction op1, Fraction op2)
{
	int num1 = op1.numerator;
	int num2 = op2.numerator;
	int denom1 = op1.denominator;
	int denom2 = op2.denominator;

	Fraction result;
	result.numerator = num1 * num2;
	result.denominator = denom1 * denom2;
...
Fraction divide(Fraction op1, Fraction op2)
{
	...
	Fraction result;
	result.numerator = num1 * denom2;
	result.denominator = denom1 * num2;
...
{
	double decimal1;
	float numerator1 = op1.numerator;
	int denominator1 = op1.denominator;
...
{
	double decimal2;
	float numerator2 = op2.numerator;
...
void printDecimals(FractionArray fractions1, FractionArray fractions2)
{
	cout << "Decimal conversion from array" << endl;
	for (int i = 0; i < fractions1.nFractions; i++)
	{
		cout << fracToDec1(fractions1.fractions[i]) << endl;
	}
	cout << endl;
	cout << "Decimal conversion from array" << endl;
	for (int i = 0; i < fractions2.nFractions; i++)
	{
		cout << fracToDec2(fractions2.fractions[i]) << endl;
	}
	cout << endl;
}
